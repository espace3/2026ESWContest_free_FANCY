...
> 앱 코드는 [`application`](https://github.com/espace3/ESW/tree/application) 브랜치를 참고하세요.

## 프로젝트 목표

- **신체 부위별 맞춤 제어** — 사용자의 머리, 상체, 하체를 인식하여 부위에 맞는 풍속을 제공
- **온디바이스 사용자 인식·추적** — 외부 네트워크 호출 없이 온디바이스로 사용자의 이동이나 자세 변화에도 대처
- **안정적인 HW 구동** — 사용자의 다양한 조작에 대하여 안정적인 제어 제공
- **스마트폰 앱 기반 제어** - 스마트폰 앱에서 다양한 동작 모드를 설정하고, 개인 저장 기능 제공
...
| **타겟** | 사용자의 상체를 화면 중앙에 붙잡는 닫힌 루프 추적 |
...
| 영상 처리 지연 (프레임 획득 → 모터 명령) | < 0.5 s | 53 ms |
| 부위 전환 정확도 | ≥ 90% | 100% (사용자 정지 상태 기준) |
| BLE 명령 처리 (수신 → 응답) | < 0.2 s | 0.5 ms |
| 동작 소음 | < 35 dB | 측정 예정 |
