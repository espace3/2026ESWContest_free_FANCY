...
CFG: dict = {
    ...
    "pins": {
        ...
        # 릴레이 IN(레벨컨버터 LV측), 물리 핀 33/35/37. GPIO9~27 대역(부팅 시
        # pull-DOWN=LOW)이라 claim 전에도 전부 오픈=정지 안전 상태가 유지된다
        # (단, 레벨컨버터 자체 풀업 유무 실측 확인 필수 — hardware/TODO.md).
        # IN4는 완전 미사용.
        "relay": {"IN1": 13, "IN2": 19, "IN3": 26},  # 미풍/약풍/강풍
    },

    # ── 릴레이 (TS0011 4ch — 선풍기 풍속 220V 탭 단속) ──────────────────────
    # 옵토 미사용. IN에 전류(HIGH) → 코일 여자 → NO-COM 닫힘 = 탭 ON.
    # 동시에 하나의 채널만 닫는다 (전부 오픈 = 정지). hardware/relay_controller.py 참고.
    "relay": {
        "active_high": True,   # 배선 확정값 — HIGH=ON. False면 안전 상태가 뒤집히므로 실배선 확인 필수.
