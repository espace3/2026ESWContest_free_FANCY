...
# 큐에 미리 쌓아두는 최대 분량 (초). 두 가지를 동시에 정하는 값이라 트레이드오프다:
#   (1) 선점 반응 지연의 상한 — 새 목표가 와도 이미 큐에 든 펄스는 취소하지 않으므로
#       이만큼은 옛 목표대로 더 간다 (팬 순항 6.75°/s 기준 0.25s ≈ 1.7°).
#   (2) 워커 스레드가 늦어도 되는 시간 — lgpio tx_pwm은 소프트웨어 타이밍이라
#       큐가 비면 펄스가 끊긴다. 추적 중에는 MoveNet 추론 3스레드 + rpicam-vid +
#       OpenCV가 같이 돌아서 워커가 쉽게 밀리는데, 0.09s로는 부족해서 큐가 비고
#       그때마다 펄스가 끊겨 "달그락" 소리가 났다 (2026-08-07 실기 확인:
#       모터 단독 구동은 어떤 조건에서도 조용했고, 카메라+추론을 동시에 돌리면
#       같은 모터 명령에서도 소리가 났다).
# 부하가 더 늘거나(두 축 동시, 웹 스트림) 소리가 다시 나면 올릴 것. 대신 (1) 때문에
# 추적이 그만큼 굼떠지므로, 소리가 안 나는 선에서 가장 낮은 값을 쓴다.
_LOOKAHEAD_S = 0.25
