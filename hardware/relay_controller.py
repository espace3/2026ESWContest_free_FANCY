...
제어수단):
  ...
  - IN에 전류를 흘리면(HIGH) 코일이 여자되어 NO-COM이 닫힘 = 해당 220V 탭 ON.
...
부팅 안전: 릴레이 핀은 GPIO9~27 대역(부팅 기본 pull-DOWN)이라 소프트웨어가
핀을 잡기 전에도 LOW=전부 오픈이 유지된다. 단, 레벨컨버터 모듈에 채널
풀업이 달려 있으면(BSS138 양방향형 등) Pi 내부 풀다운을 이겨 이 가정이
깨진다 — 부팅 직후 전 릴레이 오픈 실측 확인 필수 (hardware/TODO.md).
"""
