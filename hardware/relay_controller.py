...
부팅 안전 (⚠ 미해결): 기판이 active-LOW로 확인되면서, 릴레이 핀을 GPIO9~27
(부팅 기본 pull-DOWN) 대역에서 고른 원래 근거("LOW=오픈")는 무효가 됐다 —
LOW가 곧 ON이다. 지금은 레벨컨버터 HV측 채널 풀업(BSS138 양방향형은 10k)이
IN을 5V=OFF로 띄워주는 것에만 의존하며, 이건 아직 실측되지 않았다. claim
전(부팅 중)과 gpiochip_close() 후(종료·크래시)에 전 릴레이가 오픈인지
반드시 확인할 것 (hardware/TODO.md). 소프트웨어로는 보장할 수 없는 구간이다.
"""
