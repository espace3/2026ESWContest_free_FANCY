"""
scripts/verify_pulse_jitter.py — 스레드 웨이크업 지연 측정 (hardware/lgpio_patch.md)
...
CPU 경합이라 별도 프로세스의 스레드도 똑같이 겪는다).
    ...
    # 터미널 1 — 평소대로 추적
    python scripts/verify_E2E_v1.py --threads 3 ...

    # 터미널 2 — 그동안 재기
    python scripts/verify_pulse_jitter.py --freq 6000 --sec 10

기준선(무부하)을 먼저 떠두고 비교할 것:

    python scripts/verify_pulse_jitter.py --freq 6000 --sec 10            # 기준선
    python scripts/verify_pulse_jitter.py --freq 6000 --sec 10 --load 6   # 합성 부하(추적 대신)
    sudo chrt -f 10 $(which python3) scripts/verify_pulse_jitter.py --freq 6000 --sec 10 --load 6
    python scripts/verify_pulse_jitter.py --freq 6000 --sec 10 --cpu 3    # 코어 3 전용(어피니티 대책 예습)
